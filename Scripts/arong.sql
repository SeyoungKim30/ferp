...
SELECT s.* , PAYMENTSTATE  
FROM STOCK s ,PRODORDER p 
WHERE s.PRODUCTNUM = p.PRODUCTNUM 
AND s.FRREGINUM=p.DEMANDER 
AND STOCKDATE IN (SELECT max(STOCKDATE) FROM stock GROUP BY s.PRODUCTNUM);

SELECT sst.*, PAYMENTSTATE
FROM PRODORDER p , (SELECT * 
					FROM STOCK s 
					WHERE STOCKDATE IN (SELECT max(STOCKDATE) FROM stock GROUP BY PRODUCTNUM)) sst 
WHERE sst.PRODUCTNUM = p.PRODUCTNUM;

SELECT * 
FROM STOCK s , 
WHERE STOCKDATE IN (SELECT max(STOCKDATE) FROM stock GROUP BY PRODUCTNUM)
GROUP BY ;

SELECT s.* 
FROM STOCK s 
WHERE STOCKDATE IN (SELECT max(STOCKDATE) FROM stock GROUP BY s.PRODUCTNUM);