...
--기본 전표검색
SELECT * FROM ACSTATEMENT WHERE TO_CHAR(stmtdate,'YYYY-MM-DD') ='2023-03-03' AND FRREGINUM ='9999999999' AND STATEMENTNUM Like '%'||'WR1001'||'%' ;
--이전 전표
SELECT * FROM (
SELECT * FROM ACSTATEMENT WHERE TO_CHAR(stmtdate,'YYYY-MM-DD') ='2023-03-03' AND FRREGINUM ='9999999999' AND STATEMENTNUM = substr('WR1001',0,2)||(substr('WR1001',3,5)+1) );
--이전이후 전표번호 구하는거
SELECT  substr('WR1001',0,2)||(substr('WR1001',3,5)+1) FROM dual;
--오늘 전표번호 모두+순서대로
SELECT rownum, a.* FROM ACSTATEMENT a WHERE TO_CHAR(stmtdate,'YYYY-MM-DD') ='2023-03-03' AND FRREGINUM ='9999999999' AND LINENUM =1 ORDER BY STATEMENTNUM ;
...
--위에걸로rownum구해서 그 앞뒤 전표 뽑아오기
SELECT rownum "frn",rrnn, aa.* FROM (
	SELECT DISTINCT rrnn, a.* FROM ACSTATEMENT a, --검색하는 전표번호의 rownum 구하는거
		(SELECT rownum AS rrnn, a.STATEMENTNUM FROM ACSTATEMENT a WHERE TO_CHAR(stmtdate,'YYYY-MM-DD') ='2023-03-03' AND FRREGINUM ='9999999999' AND LINENUM =1 ORDER BY STATEMENTNUM) ii
		WHERE a.STATEMENTNUM = ii.statementNum
		AND TO_CHAR(a.stmtdate,'YYYY-MM-DD') ='2023-03-03' AND a.FRREGINUM ='9999999999'
		AND a.STATEMENTNUM ='AA1002'
		) iii , ACSTATEMENT aa
WHERE iii.statementnum(+)=aa.statementnum
AND TO_CHAR(aa.stmtdate,'YYYY-MM-DD') ='2023-03-03' AND aa.FRREGINUM ='9999999999' AND aa.LINENUM =1 ORDER BY aa.STATEMENTNUM
;
...
--전표 번호 몰라
SELECT * FROM ACSTATEMENT a WHERE TO_CHAR(stmtdate,'YYYY-MM-DD') ='2023-03-03' AND STATEMENTNUM = (SELECT min(STATEMENTNUM) FROM ACSTATEMENT a2 WHERE FRREGINUM='9999999999');
SELECT * FROM ACSTATEMENT a WHERE TO_CHAR(stmtdate,'YYYY-MM-DD') ='2023-03-03' AND FRREGINUM ='9999999999' 
	AND STATEMENTNUM = (SELECT max(STATEMENTNUM) FROM ACSTATEMENT a2 WHERE FRREGINUM='9999999999' AND TO_CHAR(stmtdate,'YYYY-MM-DD') ='2023-03-03');
--전표 수정 반복문으로 돌려가면서
